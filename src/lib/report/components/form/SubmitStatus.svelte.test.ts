...
describe('SubmitStatus', () => {
	...
	it('rendert im Zustand idle nichts', async () => {
		const { container } = await render(SubmitStatus, { state: 'idle' });

		expect(container.textContent?.trim()).toBe('');
	});

	describe('offline', () => {
		it('nennt das Schicksal der Daten', async () => {
			await render(SubmitStatus, { state: 'offline' });
		 ...
		 */
		it('meldet sich höflich per role="status", nicht per role="alert"', async () => {
			await render(SubmitStatus, { state: 'offline' });
		 ...
		 */
		it('trägt die exportierte id, auf die StepNavigation verweist', async () => {
			await render(SubmitStatus, { state: 'offline' });
	...
	describe('ARIA-Rollen', () => {
		it.each([
			...
			['offline', 'status'],
			['submitting', 'status']
		] as const)('meldet %s per role="%s"', async (state, expected) => {
			await render(SubmitStatus, { state });

			expect(block(`submit-status-${state}`)?.getAttribute('role')).toBe(expected);
		});

		it('rendert nie zwei Live-Regionen gleichzeitig', async () => {
			await render(SubmitStatus, { state: 'partial' });
	...
	describe('failed', () => {
		it('unterbricht per role="alert", weil es die Folge einer Nutzeraktion ist', async () => {
			await render(SubmitStatus, { state: 'failed' });

			expect(block('submit-status-failed')?.getAttribute('role')).toBe('alert');
		});

		it('sagt zu, dass die Eingaben erhalten bleiben', async () => {
			await render(SubmitStatus, { state: 'failed' });
		...
		it('zeigt die Referenz-ID für die Rückfrage', async () => {
			await render(SubmitStatus, { state: 'failed', referenceId: 'clx8k2p9a0001' });
		 ...
		 */
		it('nennt keine unbelegte E-Mail-Adresse', async () => {
			await render(SubmitStatus, { state: 'failed', referenceId: 'clx8k2p9a0001' });
		...
		it('zeigt keine Referenzzeile, solange keine ID vorliegt', async () => {
			await render(SubmitStatus, { state: 'failed', referenceId: '' });

			expect(block('submit-status-reference')).toBeNull();
		});

		it('macht sichtbar, der wievielte Versuch das war', async () => {
			await render(SubmitStatus, { state: 'failed', attempt: 2, maxAttempts: 3 });

			expect(block('submit-status-failed')?.textContent).toContain('Versuch 2 von 3');
		});

		it('blendet den Zähler vor dem ersten Versuch aus', async () => {
			await render(SubmitStatus, { state: 'failed', attempt: 0 });
		 ...
		 * durchsetzt.
		 */
		it('behauptet oberhalb der Bezugsgröße keine Grenze mehr', async () => {
			await render(SubmitStatus, { state: 'failed', attempt: 4, maxAttempts: 3 });
		...
		it('setzt die übergebene Überschrift ein', async () => {
			await render(SubmitStatus, { state: 'failed', title: 'Zu viele Übermittlungen' });
		...
		it('löst über die Schaltfläche einen erneuten Versuch aus', async () => {
			const onRetry = vi.fn();
			await render(SubmitStatus, { state: 'failed', onRetry });
	...
	describe('partial', () => {
		 ...
		 */
		it('gibt keine uneingeschränkte Datenzusage', async () => {
			await render(SubmitStatus, { state: 'partial' });
		 ...
		 */
		it('nennt genau die Frist, die auch tatsächlich angewendet wird', async () => {
			await render(SubmitStatus, { state: 'partial' });
		 ...
		 * ins Leere.
		 */
		it('nennt auch hier den Grund des Fehlschlags', async () => {
			await render(SubmitStatus, {
				state: 'partial',
	...
	describe('Ausweg aus dem Offline-Zustand', () => {
		it('bietet einen Versuch an, wenn der Aufrufer einen Handler übergibt', async () => {
			const onRetry = vi.fn();
			await render(SubmitStatus, { state: 'offline', onRetry });
		...
		it('zeigt keine Schaltfläche, wenn die Sperre sicher ist', async () => {
			await render(SubmitStatus, { state: 'offline' });
	...
	describe('submitting', () => {
		it('trägt keine Statusfarbe — ein Ladevorgang ist keine Warnung', async () => {
			await render(SubmitStatus, { state: 'submitting' });
