...
```
src/routes/
├── +page.svelte           # Multi-Step Container
└── components/
    ├── steps/             # Step Components
    │   ├── Step1.svelte   # Position & Zeit
    │   ├── Step2.svelte   # Details
    │   ├── Step3.svelte   # Verhalten
    │   └── Step4.svelte   # Kontakt
...
Schema in `src/lib/sightingSchema.ts`:
...
export const sightingSchema = yup.object({
    // Pflichtfelder
    lat: yup.number()
        .required('Position erforderlich')
        .min(-90).max(90),
    lng: yup.number()
        .required('Position erforderlich')
        .min(-180).max(180),
    ...
    species: yup.string()
        .required('Tierart erforderlich'),
    count: yup.number()
        .positive('Mindestens 1')
        .integer()
        .required(),
...
/>
<div id="species-help" class="text-sm text-base-content/70">
    Wähle die beobachtete Tierart
</div>
{#if $errors.species && $touched.species}
