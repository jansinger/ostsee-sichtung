...
### Was fehlt noch
- 🔄 **Database Storage**: Wetterdaten werden aktuell nur in der UI angezeigt, nicht gespeichert
- 🔄 **Forecast vs. Historical**: Keine Unterscheidung zwischen aktuellen (Forecast) und historischen Daten
- 🔄 **Caching/Deduplication**: Mehrfache API-Calls für gleiche Position/Datum
- 🔄 **Admin Interface**: Keine Anzeige der Wetterdaten im Admin-Bereich
...
export class OpenMeteoProvider implements WeatherProvider {
  ...
  async getHistoricalWeather(
    ...
    const weatherUrl = `https://archive-api.open-meteo.com/v1/era5?` +
...
<script lang="ts">
  ...
  const dataSourceInfo = $derived(() => {
    ...
    return {
      ...
      warning: weatherData.data_type === 'forecast' && isToday() ? 
        'Die Wetterdaten basieren auf Vorhersagen, da die Sichtung heute stattfand.' : null
    };
  });
  ...
  <div class="weather-comparison">
    ...
    {#if weatherData && dataSourceInfo}
      <div class="api-data {dataSourceInfo.className}">
        ...
        <p>Wind: {weatherData.processed.windDirection}° {weatherData.processed.windDirectionCardinal}</p>
        ...
        {#if canRefresh}
          <button onclick={refreshWeatherData} disabled={isRefreshing} 
  ...
  <div class="weather-source-info">
    ...
    {:else}
      <div class="alert alert-success alert-sm">
        <span class="text-xs">
          📊 Historische Wetterdaten für vergangenes Datum verfügbar.
        </span>
      </div>
    {/if}
  </div>
